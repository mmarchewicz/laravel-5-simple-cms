...
<div class="container">
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-import" method="POST" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postImport') ?>" data-remote="true" role="form">
        ...
        <form class="form-find" method="POST" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Publish translations</button>
                <a href="<?= action('\App\Http\Controllers\Admin\TranslationsController@getIndex') ?>" class="btn btn-default">Back</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
    ...
    <?php if($group): ?>
        <form action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
        ...
        <table class="table">
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postDelete', [$group, $key]) ?>"
                               class="delete-key"
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postRemoveLocale') ?>" data-confirm="Are you sure to remove this locale and all of data?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postAddLocale') ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <p>
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\App\Http\Controllers\Admin\TranslationsController@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\App\Http\Controllers\Admin\TranslationsController@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\App\Http\Controllers\Admin\TranslationsController@getIndex') ?>';
                }
            });
